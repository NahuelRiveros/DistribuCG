...
export async function obtenerRecaudacionDetalleDia({ anio, mes, dia }) {
  ...
  const sql = `
    ...
    WHERE f.fecha_inicio = :fecha::date AND COALESCE(f.monto_pagado, 0) > 0
...
export async function obtenerPlanesPopulares({ anio } = {}) {
  ...
  const filtroAnio = anioNum ? `AND EXTRACT(YEAR FROM m.fecha_inicio) = ${anioNum}` : "";
  ...
  const sql = `
    ...
    FROM membresia m
    LEFT JOIN plan_tipo tp ON tp.id = m.plan_tipo_id
