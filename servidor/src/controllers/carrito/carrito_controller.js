...
} from "../../services/carrito/carrito_service.js";
...
export async function agregarItemController(req, res) {
  try {
    ...
    const data = await agregarItem(req.user.usuario_id, { producto_id, variante_id, cantidad });
    return res.status(201).json({ ok: true, mensaje: "Producto agregado al carrito", data });
  } catch (error) {
...
export async function eliminarItemController(req, res) {
  try {
    ...
    return res.json({ ok: true, mensaje: "Producto eliminado del carrito", data });
  } catch (error) {
...
export async function vaciarCarritoController(req, res) {
  try {
    ...
    return res.json({ ok: true, mensaje: "Carrito vaciado", data: null });
  } catch (error) {
    console.error("Error al vaciar carrito:", error);
    return res.status(500).json({ ok: false, mensaje: "Error interno al vaciar el carrito" });
  }
}
